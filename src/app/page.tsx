      ...
      <main className="max-w-7xl mx-auto px-4 py-4">
        ...
        {tab==="instagram"&&(
          <div>
            ...
            {adModal&&(
              <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={()=>setAdModal(null)}>
                <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 max-w-md w-full mx-4" onClick={e=>e.stopPropagation()}>
                  <h3 className="font-bold mb-1">🚀 Crear Publicidad desde Reel</h3>
                  <p className="text-gray-400 text-xs mb-3 truncate">{adModal.hook}</p>
                  <div className="bg-gray-700/30 rounded-lg p-3 mb-4 text-xs">
                    <p>Reach: {adModal.reach?.toLocaleString()} · Saves: {adModal.saves} · Eng: {adModal.engagementRate}%</p>
                  </div>
                  <p className="text-sm text-gray-300 mb-3">Este reel tiene buen rendimiento organico. Para usarlo como ad tienes dos opciones:</p>
                  <div className="space-y-2 mb-4">
                    <a href={adModal.permalink} target="_blank" className="block w-full bg-blue-600 hover:bg-blue-700 px-4 py-3 rounded-lg text-sm font-medium text-center">📸 Boost directo desde Instagram ↗</a>
                    <a href={`https://adsmanager.facebook.com/adsmanager/manage/ads?act=289250686730282`} target="_blank" className="block w-full bg-purple-600 hover:bg-purple-700 px-4 py-3 rounded-lg text-sm font-medium text-center">🎯 Crear ad en Ads Manager ↗</a>
                  </div>
                  <div className="text-xs text-gray-400 mb-3">
                    <p className="font-medium text-gray-300 mb-1">📋 Pasos en Ads Manager:</p>
                    <p>1. Selecciona campana existente o crea nueva</p>
                    <p>2. En "Ad", elige "Usar publicacion existente"</p>
                    <p>3. Busca este reel por fecha: {adModal.date?.substring(0,10)}</p>
                  </div>
            ...
            {igLoading&&<div className="text-center py-12"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-500 mx-auto mb-4"/><p className="text-gray-400">Analizando publicaciones con insights...</p></div>}
            {igData&&(
              <>
                {/* Header */}
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="font-bold text-lg">@{igData.account?.username}</p><p className="text-gray-400 text-sm">{igData.account?.followers?.toLocaleString()} seguidores · {igData.posts?.length} posts cargados</p></div>
                  </div>
                  <div className="flex gap-2 items-center flex-wrap">
                    ...
                    <button onClick={()=>{setTab("scripts");setScriptTheme(igFilter!=="ALL"?igFilter:"");}} className="bg-green-600/80 hover:bg-green-700 px-3 py-1.5 rounded-lg text-xs font-medium">📝 Guiones</button>
                    <button onClick={loadHistory} disabled={historyLoading} className="bg-gray-600 hover:bg-gray-700 px-3 py-1.5 rounded-lg text-xs font-medium disabled:opacity-50">{historyLoading?"Cargando...":"📜 Cargar más historico"}</button>
                  </div>
                </div>

                {/* Sub-tabs: Live vs Library */}
                <div className="flex gap-2 mb-4">
                  <button onClick={()=>setIgSubTab("live")} className={`px-4 py-2 rounded-lg text-sm font-medium ${igSubTab==="live"?"bg-purple-600":"bg-gray-800 hover:bg-gray-700"}`}>📊 Analisis Live ({igData.posts?.length})</button>
                  <button onClick={()=>setIgSubTab("library")} className={`px-4 py-2 rounded-lg text-sm font-medium ${igSubTab==="library"?"bg-purple-600":"bg-gray-800 hover:bg-gray-700"}`}>📚 Biblioteca ({library.length})</button>
                  {selectedPosts.size>0&&(
                    <div className="flex items-center gap-2 ml-auto bg-blue-600/20 border border-blue-500/30 rounded-lg px-3 py-1">
                      <span className="text-xs">{selectedPosts.size} seleccionados</span>
                      <div className="flex gap-1">
                        {CTA_OPTIONS.map(cta=>(
                          <button key={cta.id} onClick={()=>addSelectedToLibrary(cta.id)} className={`${cta.color} px-2 py-0.5 rounded text-[10px] font-medium`} title={`Guardar como ${cta.label}`}>{cta.label.split(" ")[0]}</button>
                        ))}
                      </div>
                    </div>
                  )}
                ...
                {igSubTab==="live"&&(
                  <div className="bg-gray-800/60 rounded-xl border border-gray-700/50 overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead><tr className="text-gray-400 text-xs border-b border-gray-700/50">
                        <th className="p-2 w-8"><input type="checkbox" onChange={e=>{if(e.target.checked){const all=new Set<string>(igData.posts.filter((p:any)=>igFilter==="ALL"||p.theme===igFilter).map((p:any)=>p.id));setSelectedPosts(all);}else{setSelectedPosts(new Set());}}} className="rounded"/></th>
                        <th className="text-left p-3">Contenido</th><th className="p-2">Tema</th><th className="text-right p-2">Reach</th><th className="text-right p-2">Saves</th><th className="text-right p-2">Shares</th><th className="text-right p-2">Eng%</th><th className="p-2">Potencial</th><th className="p-2">Acciones</th>
                      </tr></thead>
                      <tbody>
                        {(igData.posts||[]).filter((p:any)=>igFilter==="ALL"||p.theme===igFilter).map((p:any,i:number)=>{
                          ...
                          const inLibrary=library.some((l:any)=>l.id===p.id);
                          return(
                            <tr key={i} className={`border-t border-gray-700/30 hover:bg-gray-700/20 ${selectedPosts.has(p.id)?"bg-blue-500/10":""}`}>
                              <td className="p-2"><input type="checkbox" checked={selectedPosts.has(p.id)} onChange={()=>toggleSelect(p.id)} className="rounded"/></td>
                              <td className="p-3 max-w-xs"><p className="font-medium text-sm truncate">{p.hook||p.caption?.substring(0,80)}</p><p className="text-gray-500 text-xs">{p.date?.substring(0,10)} · {p.likes} likes · {p.comments} comm</p></td>
                              ...
                              <td className="p-2"><div className="flex gap-1">
                                {!inLibrary&&<Btn label="📚" icon="" color="bg-purple-600/80" onClick={()=>addToLibrary(p)} small/>}
                                {inLibrary&&<span className="text-xs text-green-400">✓ Guardado</span>}
                                <Btn label="🚀" icon="" color="bg-blue-600/80" onClick={()=>setAdModal(p)} small/>
                                <a href={p.permalink} target="_blank" className="text-blue-400 hover:text-blue-300 px-1">↗</a>
                              </div></td>
                            </tr>
                          );
                        })}
                ...
                {igSubTab==="library"&&(
                  <div>
                    {library.length===0?(
                      <div className="text-center py-12 bg-gray-800/60 rounded-xl border border-gray-700/50">
                        <p className="text-4xl mb-3">📚</p>
                        <p className="text-gray-400">Tu biblioteca esta vacia. Guarda posts desde el tab "Analisis Live" con el boton 📚</p>
                      </div>
                    ):(
                      <>
                        {/* CTA Filter */}
                        <div className="flex gap-2 mb-4 flex-wrap">
                          <button onClick={()=>setIgFilter("ALL")} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${igFilter==="ALL"?"bg-purple-600":"bg-gray-700"}`}>Todos ({library.length})</button>
                          {CTA_OPTIONS.map(cta=>{const count=library.filter((p:any)=>p.cta===cta.id).length;return count>0&&(
                            <button key={cta.id} onClick={()=>setIgFilter(cta.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${igFilter===cta.id?"bg-purple-600":"bg-gray-700"}`}>{cta.label} ({count})</button>
                          );})}
                          <button onClick={()=>setIgFilter("SIN_CTA")} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${igFilter==="SIN_CTA"?"bg-purple-600":"bg-gray-700"}`}>Sin CTA ({library.filter((p:any)=>!p.cta).length})</button>
                          <button onClick={()=>setIgFilter("AD_READY")} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${igFilter==="AD_READY"?"bg-green-600":"bg-gray-700"}`}>🚀 Listos para Ad ({library.filter((p:any)=>p.markedForAd).length})</button>
                        </div>

                        <div className="bg-gray-800/60 rounded-xl border border-gray-700/50 overflow-x-auto">
                          <table className="w-full text-sm">
                            <thead><tr className="text-gray-400 text-xs border-b border-gray-700/50">
                              <th className="text-left p-3">Contenido</th><th className="p-2">CTA</th><th className="text-right p-2">Reach</th><th className="text-right p-2">Saves</th><th className="text-right p-2">Shares</th><th className="text-right p-2">Eng%</th><th className="p-2">Ad?</th><th className="p-2">Acciones</th>
                            </tr></thead>
                            <tbody>
                              {library.filter((p:any)=>{
                                if(igFilter==="ALL")return true;if(igFilter==="SIN_CTA")return !p.cta;if(igFilter==="AD_READY")return p.markedForAd;return p.cta===igFilter;
                              }).sort((a:any,b:any)=>b.saves-a.saves).map((p:any,i:number)=>(
                                <tr key={i} className={`border-t border-gray-700/30 hover:bg-gray-700/20 ${p.markedForAd?"bg-green-500/5":""}`}>
                                  <td className="p-3 max-w-xs"><p className="font-medium text-sm truncate">{p.hook||p.caption?.substring(0,80)}</p><p className="text-gray-500 text-xs">{p.date?.substring(0,10)} · Guardado: {p.addedAt?.substring(0,10)}</p>{p.notes&&<p className="text-yellow-400/70 text-xs mt-0.5">📝 {p.notes}</p>}</td>
                                  <td className="p-2">
                                    <select value={p.cta||""} onChange={e=>updateLibraryItem(p.id,{cta:e.target.value})} className="bg-gray-700 border border-gray-600 rounded px-1 py-0.5 text-xs w-20">
                                      <option value="">-</option>
                                      {CTA_OPTIONS.map(c=><option key={c.id} value={c.id}>{c.id}</option>)}
                                    </select>
                                  </td>
                                  <td className="p-2 text-right">{p.reach?.toLocaleString()}</td>
                                  <td className="p-2 text-right font-bold text-yellow-400">{p.saves}</td>
                                  ...
                                  <td className="p-2 text-center">
                                    <input type="checkbox" checked={p.markedForAd||false} onChange={e=>updateLibraryItem(p.id,{markedForAd:e.target.checked})} className="rounded"/>
                                  </td>
                                  <td className="p-2"><div className="flex gap-1">
                                    <Btn label="🚀" icon="" color="bg-blue-600/80" onClick={()=>setAdModal(p)} small/>
                                    <a href={p.permalink} target="_blank" className="text-blue-400 hover:text-blue-300 px-1">↗</a>
                                    <Btn label="🗑" icon="" color="bg-red-600/60" onClick={()=>removeFromLibrary(p.id)} small/>
                                  </div></td>
                                </tr>
                              ))}
                            </tbody>
